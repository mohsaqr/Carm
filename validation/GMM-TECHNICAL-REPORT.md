...
**Dependencies:** Zero external math libraries — all linear algebra, distributions, EM algorithms, and distance computations from scratch
...
- **Distributions** (`core/math.ts`): chi-square, normal, F, t — CDF and inverse CDF
- **Distance computations**: Euclidean distance matrix via `Float64Array` for O(n²) pairwise computation
...
Rather than storing and inverting full D×D covariance matrices, the GMM implementation stores the **eigendecomposition** Σ = UΛU' per component. This provides:

- O(D²) log-PDF evaluation via Mahalanobis distance in the rotated basis
...
- All return types are `readonly` — results are immutable
...
where:
- π_k are the mixing weights (Σ π_k = 1, π_k > 0)
- μ_k are the D-dimensional component means
- Σ_k are the D×D component covariance matrices
- 𝒩(x | μ, Σ) is the multivariate normal density

### 2.2 Expectation-Maximization

**Implementation:** `fitGMM()` at lines 263–494 of `clustering.ts`.

The EM algorithm iterates between two steps until convergence:

**E-step** (lines 315–325): Compute posterior responsibilities z_ik — the probability that observation i was generated by component k:

```
z_ik = π_k · 𝒩(x_i | μ_k, Σ_k) / Σ_j π_j · 𝒩(x_i | μ_j, Σ_j)
```

All computations are performed in **log-space** to prevent underflow. The log-posterior is:

```
log z_ik = log π_k + log 𝒩(x_i | μ_k, Σ_k) - logSumExp_j[log π_j + log 𝒩(x_i | μ_j, Σ_j)]
```

The observed-data log-likelihood is accumulated as a byproduct:

```
ℓ(θ) = Σ_i log[Σ_k π_k · 𝒩(x_i | μ_k, Σ_k)] = Σ_i logSumExp_k[log π_k + log 𝒩(x_i | μ_k, Σ_k)]
```

**M-step** (lines 334–436): Update parameters to maximize the expected complete-data log-likelihood:

```
N_k = Σ_i z_ik                    (effective count for component k)
π_k = N_k / N                     (updated weight)
μ_k = (Σ_i z_ik · x_i) / N_k     (weighted mean)
Σ̂_k = (Σ_i z_ik · (x_i - μ_k)(x_i - μ_k)') / N_k    (weighted covariance)
```

After computing the empirical covariance Σ̂_k, the covariance model constraint is applied (see Section 3), followed by regularization: Σ_k[j,j] += `regCovar` (default 1e-6).

**Convergence** (line 327): The algorithm stops when |ℓ(θ_t) - ℓ(θ_{t-1})| < `tol` (default 1e-6), or after `maxIter` iterations (default 200).

### 2.3 Multivariate Normal Log-PDF via Eigendecomposition

**Implementation:** `mvnLogPdf()` at lines 215–235.

Given the eigendecomposition Σ_k = U_k Λ_k U_k', the log-PDF is:

```
log 𝒩(x | μ, Σ) = -½[D·log(2π) + Σ_j log(λ_j) + Σ_j y_j²/λ_j]
```

where y = U'(x - μ) is the projection onto the eigenbasis. This avoids explicit matrix inversion — the Mahalanobis distance (x-μ)'Σ⁻¹(x-μ) = Σ_j y_j²/λ_j uses only element-wise division by eigenvalues.

The eigenvector matrix U is stored as a flat row-major `number[]` for cache-friendly access during the inner loop. The projection y_i = Σ_j U[j,i] · (x_j - μ_j) runs in O(D²) per observation per component.

### 2.4 Initialization

**K-Means++ initialization** (lines 168–206): Each GMM run begins with K-Means++ center selection:

1. Select the first center uniformly at random from the data
2. For each subsequent center, compute D²(x) = min distance² to existing centers
3. Select the next center with probability proportional to D²(x)

Initial weights are set to 1/K, and eigenvalues are initialized from the global variance `σ² = Σ_{i,j} (x_{ij} - x̄_j)² / (N·D)`. This provides a reasonable starting point for EM.
...
During EM, a component can lose all responsibility (N_k → 0). The M-step guards against this with `nk = Math.max(N_k, MIN_PROB)` where `MIN_PROB = 1e-300`, preventing division by zero while allowing the component to be revived in subsequent iterations as the other components shift.

### 2.7 Prediction

**Implementation:** `predictGMM()` at lines 499–535.
...
where λ_k controls volume, D_k controls orientation, and A_k controls shape. The six models constrain these components:

### 3.1 VVV — Variable Volume, Shape, and Orientation

**Full unconstrained covariance** per component. Each Σ_k is a free D×D positive-definite matrix.

```
Σ_k = U_k Λ_k U_k'    (full eigendecomposition)
```

**Implementation** (lines 390–393): Eigendecompose the empirical covariance, clamp eigenvalues to `regCovar`.

**Parameters per component:** D(D+1)/2. **Total covariance parameters:** K·D(D+1)/2.

### 3.2 EEE — Equal Volume, Shape, and Orientation

**Single pooled covariance** shared across all components: Σ_k = Σ for all k.

```
Σ = Σ_k π_k · Σ̂_k    (weighted average of per-component covariances)
```

**Implementation** (lines 340–343, 408–417): Accumulate weighted covariances during M-step, then eigendecompose the pool and assign to all components.

**Total covariance parameters:** D(D+1)/2.

### 3.3 VVI — Variable Volume and Shape, Identity Orientation

**Diagonal covariance** per component. Off-diagonal elements are zero; each variable has its own variance per component.

```
Σ_k = diag(σ²_{k1}, σ²_{k2}, ..., σ²_{kD})
```

**Implementation** (lines 394–397): Extract diagonal of empirical covariance, set U = I.

**Parameters per component:** D. **Total covariance parameters:** K·D.

### 3.4 EEI — Equal Volume and Shape, Identity Orientation

**Single shared diagonal covariance:** Σ_k = diag(σ²_1, ..., σ²_D) for all k.

**Implementation** (lines 418–425): Pool diagonals, assign shared values.

**Total covariance parameters:** D.

### 3.5 VII — Variable Volume, Identity Shape and Orientation

**Spherical covariance** per component. Each component is an isotropic Gaussian with its own variance.

```
Σ_k = σ²_k · I    where σ²_k = tr(Σ̂_k) / D
```

**Implementation** (lines 398–403): Compute trace of empirical covariance, divide by D, fill eigenvalues.

**Parameters per component:** 1. **Total covariance parameters:** K.

### 3.6 EII — Equal Volume, Identity Shape and Orientation

**Single shared spherical covariance:** Σ_k = σ² · I for all k.

```
σ² = tr(Σ_pool) / D
```

**Implementation** (lines 426–434): Pool covariances, compute shared trace/D.
...
|-------|--------|-------|-------------|-------------|----------|
| VVV | Variable | Variable | Variable | K·D(D+1)/2 | (K-1) + K·D + K·D(D+1)/2 |
| EEE | Equal | Equal | Equal | D(D+1)/2 | (K-1) + K·D + D(D+1)/2 |
| VVI | Variable | Variable | Axis-aligned | K·D | (K-1) + K·D + K·D |
| EEI | Equal | Equal | Axis-aligned | D | (K-1) + K·D + D |
| VII | Variable | Spherical | — | K | (K-1) + K·D + K |
| EII | Equal | Spherical | — | 1 | (K-1) + K·D + 1 |

The total degrees of freedom (line 468) is: `df = (K-1) + K·D + covariance_params`.
...
**Implementation:** `kMeansPlusPlus()` at lines 168–206.
...
2. For j = 2, ..., K:
   a. For each point x_i, compute D²(x_i) = min_{c ∈ existing} ||x_i - c||²
   b. Select c_j = x_i with probability D²(x_i) / Σ_i D²(x_i)
3. Return {c_1, ..., c_K}
```

### 4.2 Distance Tracking Optimization

Rather than recomputing all pairwise distances at each step, the implementation maintains a running `dists` array (Float64Array) with the minimum squared distance from each point to any selected center. After adding center j, only distances to center j need to be computed; the minimum is taken with the existing `dists[i]`. This reduces initialization from O(N·K²) to O(N·K).
...
With D=10 dimensions and well-separated clusters, the multivariate normal density can reach values as small as exp(-500), which is far below `Number.MIN_VALUE ≈ 5e-324`. Direct computation of p(x_i | k) would produce 0.0, making the posterior z_ik = 0/0 undefined.

### 5.2 The logSumExp Trick

**Implementation:** `logSumExp()` at lines 39–48.
...
By subtracting the maximum, all exponents are ≤ 0, so exp(a_k - max(a)) ∈ (0, 1]. The sum is then ≥ 1, so its logarithm is non-negative. This gives full precision across the entire range of floating-point numbers.

**Applied in:**
- GMM E-step: posterior normalization (line 320)
- LCA E-step: class membership posteriors (line 624)
- LTA forward algorithm: α recursion (line 790)
- LTA backward algorithm: β recursion (line 808)
...
1. Prevents singular covariance (det(Σ) = 0) which would produce log-PDF = -∞
...
Lower BIC indicates a better balance between fit (log-likelihood) and complexity (degrees of freedom). BIC penalizes complexity more heavily than AIC for N > e² ≈ 7.4, making it the standard criterion for mixture model selection.
...
where E_raw = -Σ_i Σ_k z_ik · log(z_ik) is the raw classification entropy. ICL penalizes models with ambiguous classifications — even if the log-likelihood is high, overlapping components receive a penalty. This makes ICL prefer well-separated solutions.

### 6.4 Automatic Model Selection

**`findBestGMM()`** (lines 546–564) performs a grid search over K values and covariance model types, returning the model with the lowest BIC:
...
**`fitGMMRange()`** (lines 1085–1101) fits GMM at each K in a range with a fixed covariance model, returning an array of results for BIC curve plotting and elbow analysis.
...
**Implementation:** `computeNormalizedEntropy()` at lines 62–69.
...
where E_raw = -Σ_i Σ_k z_ik · log(z_ik). When all posteriors are one-hot (perfect classification), E_raw = 0 and E_norm = 1. When all posteriors are uniform (K=1/K), E_raw = N·log(K) and E_norm = 0.
...
**Implementation:** `computeAvePP()` at lines 71–83.

For each component k, AvePP measures the average confidence of observations assigned to that component:

```
AvePP_k = (Σ_{i: argmax_j z_ij = k} max_j z_ij) / |{i: argmax_j z_ij = k}|
```

AvePP ≥ 0.7 per component is considered acceptable (Nagin, 2005). Values near 1.0 indicate well-separated components; values near 1/K indicate assignments are no better than random.
...
```
E_i = 1 + Σ_k z_ik · log(z_ik) / log(K)
```

E_i ∈ [0, 1], where 1 = unambiguous assignment. The mean of case-specific entropies equals the overall normalized entropy (verified in tests at 10 decimal places).
...
**Implementation:** `fitLCA()` at lines 585–691.

LCA is a mixture model for binary (0/1) data. Each latent class k has item-response probabilities ρ_km = P(x_m = 1 | class k):
...
**E-step** (lines 615–627): The log-likelihood for observation i under class k is:

```
ℓ_ik = log π_k + Σ_m [x_im · log ρ_km + (1-x_im) · log(1-ρ_km)]
```

Posteriors are computed via logSumExp normalization.

**M-step** (lines 641–652):

```
π_k = N_k / N
ρ_km = (Σ_i z_ik · x_im) / N_k
```

Item-response probabilities are clamped to [1e-10, 1-1e-10] to prevent log(0). This uses **raw MLE** without Beta prior smoothing — matching R's poLCA package exactly.

**Key learning:** An earlier implementation used Beta(1,1) smoothing: ρ = (sumX + 1) / (sumW + 2). This biased ρ toward 0.5 and caused ΔLL ≈ 0.478 vs poLCA on a 2-class, 5-item dataset (N=100). Switching to raw MLE with minimal floor achieved exact match to 10 decimal places (ΔLL = 7.6e-9).

### 8.3 Initialization

Item-response probabilities ρ are initialized uniformly random in (0.1, 0.9) via the seeded PRNG (line 600). Class weights start at 1/K. This matches poLCA's random initialization strategy.
...
**Implementation:** `fitLTA()` at lines 729–950.
...
- **π_s**: Initial state distribution (K probabilities)
- **τ_jl**: Transition probabilities P(s_t = l | s_{t-1} = j) (K×K matrix)
- **ρ_sm**: Item-response probabilities P(x_m = 1 | state s) (K×M, time-invariant)
...
**Forward pass** (lines 779–792):

```
log α_0(s) = log π_s + log B_0(s)
log α_t(s) = log B_t(s) + logSumExp_p[log α_{t-1}(p) + log τ_{ps}]
```

**Backward pass** (lines 798–810):

```
log β_{T-1}(s) = 0
log β_t(s) = logSumExp_n[log τ_{sn} + log B_{t+1}(n) + log β_{t+1}(n)]
```

**State posteriors** (lines 812–819):

```
γ_t(s) = exp(log α_t(s) + log β_t(s) - log p(x))
```

**Transition posteriors** (lines 822–838):

```
ξ_t(j,l) ∝ exp(log α_t(j) + log τ_{jl} + log B_{t+1}(l) + log β_{t+1}(l))
```

### 9.3 M-Step with Smoothing

The M-step updates (lines 863–878) use light smoothing to prevent degenerate parameters:

```
π_s = (Σ_i γ_0(s)_i + 1) / (N + K)                    [Dirichlet(1) prior]
τ_jl = (Σ_i Σ_t ξ_t(j,l)_i + 0.1) / (Σ_i Σ_t γ_t(j)_i + 0.1·K)    [light smoothing]
ρ_sm = clip(Σ_i Σ_t γ_t(s)_i · x_{itm} / Σ_i Σ_t γ_t(s)_i, [1e-10, 1-1e-10])
```

The Dirichlet(1) prior on π ensures no state has zero initial probability. The 0.1 smoothing on τ prevents absorbing states. The clip on ρ prevents log(0).

### 9.4 Viterbi Decoding

**Implementation:** lines 881–912.

The most likely state sequence for each subject is found via dynamic programming:

```
vt_0(s) = log π_s + log B_0(s)
vt_t(s) = log B_t(s) + max_p[vt_{t-1}(p) + log τ_{ps}]
ptr_t(s) = argmax_p[vt_{t-1}(p) + log τ_{ps}]
```
...
This counts: initial state probabilities (K-1), transition probabilities (K rows of K-1 free probabilities each), and item-response probabilities (K·M).

### 9.6 Entropy for LTA

Posteriors γ are flattened across subjects and timepoints (N·T rows × K columns), then the standard mclust entropy formula is applied (lines 918–926). This measures classification certainty across all observation-timepoint pairs.
...
**Implementation:** `runKMeans()` at lines 967–1054.
...
2. Repeat until convergence:
   ...
   c. Convergence: stop if Σ_k ||c_k^new - c_k^old||² < tol
```

### 10.2 Empty Cluster Re-seeding

**Implementation:** lines 1014–1030.
...
```
farIdx = argmax_i ||x_i - c_{labels[i]}||²
c_empty = x_{farIdx}
```
...
```
W = Σ_i ||x_i - c_{labels[i]}||²
```

### 10.4 Prediction

**`predictKMeans()`** (lines 1128–1146): Assigns new data to the nearest centroid.

### 10.5 Range Fitting

**`fitKMeansRange()`** (lines 1111–1126): Fits K-Means at each K, returns sorted array for elbow method plotting.
...
**Implementation:** `runDBSCAN()` at lines 1307–1400.
...
2. Find the ε-neighborhood of each point: N_ε(i) = {j : d(i,j) ≤ ε}
3. Classify each point:
   - **Core**: |N_ε(i)| ≥ minPts
   - **Border**: not core, but in the ε-neighborhood of a core point
...
**`kDistancePlot()`**: For each point, computes the distance to its k-th nearest neighbor. Sorting these distances and plotting them helps identify the "elbow" — the appropriate ε value.
...
**Implementation:** `runHierarchical()` at lines 1489–1834.

HAC builds a dendrogram by iteratively merging the two closest clusters. The distance between a newly merged cluster (i ∪ j) and any other cluster k is computed via the Lance-Williams formula:
...
**Implementation:** lines 1770–1835.
...
**Implementation:** lines 1739–1762.
...
**Implementation:** `silhouetteScores()` at lines 1199–1262.

For each non-noise point i in cluster C_i:

```
a(i) = mean distance to all other points in C_i     (cohesion)
b(i) = min_{C ≠ C_i} mean distance to points in C   (separation)
s(i) = (b(i) - a(i)) / max(a(i), b(i))              ∈ [-1, 1]
```

- s(i) ≈ 1: well-assigned (far from other clusters, close to own)
...
**Implementation:** `PRNG` class at lines 23–35.
...
Properties:
- **Period:** ~2³² (~4.3 billion values)
- **Speed:** Single 32-bit multiplication per call, no state array
- **Determinism:** Identical seed → identical sequence, across all platforms
- **Default seed:** 42
...
|----------|-----------|-----------|
| Log-likelihood (synthetic) | ±2.0 | Different init strategies (K-Means++ vs hierarchical) |
| Log-likelihood (real) | ±5.0 | mclust uses priorControl() conjugate prior |
| BIC | ±10 | Follows from LL tolerance |
| Means | ±0.5 (synthetic), ±0.1 (real) | Order-invariant comparison |
| Weights | ±0.05 | Sorted comparison |
| Entropy | ±0.02 | Sensitive to minor parameter differences |
| AvePP | ±0.03 | Per-component, sorted |
| Classification agreement | ≥90% (LCA), ≥95% (GMM/KMeans) | After optimal relabeling |
| K-Means inertia | <5% relative | Both implementations use Lloyd's |
| LCA rho | ±0.05 | Item-response probabilities |
...
**Dataset:** 4 Gaussian clusters in 3D, N=50 per cluster, means at (0,0,0), (5,0,3), (0,5,5), (5,5,0), covariance = 0.5·I.

| Metric | Model | R mclust | Carm | Δ |
|--------|-------|----------|------|---|
...
|--------|----------------|----------------------|-----------|------|
| Log-likelihood | -342.0091 | Within 2.0 | ±2.0 | ✓ |
| BIC | 760.52 | Within 10 | ±10 | ✓ |
| Entropy | 0.9879 | Within 0.02 | ±0.02 | ✓ |
| AvePP | [0.991, 0.997, 0.999] | All > 0.7 | ≥0.7 | ✓ |
| Classification | — | ≥95% agreement | ≥95% | ✓ |
...
|--------|---------|----------------------|-----------|------|
| Log-likelihood | -260.6621 | Within 1.0 | ±1.0 | ✓ |
| BIC | 571.98 | Within 5.0 | ±5.0 | ✓ |
| AIC | 543.32 | Within 5.0 | ±5.0 | ✓ |
| Item-response ρ | see below | Within 0.05 | ±0.05 | ✓ |
| Weights | [0.483, 0.517] | Within 0.05 | ±0.05 | ✓ |
| Classification | — | ≥90% agreement | ≥90% | ✓ |

**Critical finding:** Achieving exact LCA match required using raw MLE without Beta smoothing. The ΔLL dropped from 0.478 (with smoothing) to 7.6×10⁻⁹ (without).

### 16.4 K-Means (N=90, K=3)

| Metric | R stats::kmeans | Carm (best of 7 seeds) | Tolerance | Pass |
|--------|-----------------|----------------------|-----------|------|
| Inertia | 168.9306 | Within 5% | <5% relative | ✓ |
| Centroids | [3 centers] | Within 0.5 | ±0.5 | ✓ |
| Classification | — | ≥95% agreement | ≥95% | ✓ |

### 16.5 K-Means: 4-Cluster 3D Data (N=200)

| Metric | R stats::kmeans | Carm | Δ |
|--------|-----------------|------|---|
| Inertia | 299.5402 | 299.5402 | **0.00%** |
| Converged | yes (4 iters) | yes (1 iter) | — |
...
| Test | Tolerance | Pass |
|------|-----------|------|
| Normalized entropy matches R | ±0.02 | ✓ |
| Entropy ∈ [0, 1] | — | ✓ |
| Case-specific entropies average to overall | 1e-10 | ✓ |
| AvePP ≥ 0.7 per component | — | ✓ |
| ICL = BIC + 2·rawE | 1e-6 | ✓ |
| ICL matches R | ±15 | ✓ |
...
|---------|-------------|-----------|------|
| Ward.D2 | All 29 heights | 1e-10 | ✓ |
| Single | All 29 heights | 1e-10 | ✓ |
| Complete | All 29 heights | 1e-10 | ✓ |
| Average | All 29 heights | 1e-10 | ✓ |

HAC is fully deterministic — heights match R to machine precision.
...
| Metric | R mclust | Carm | Δ | Note |
|--------|----------|------|---|------|
| Log-likelihood | -2782.9807 | -2782.3529 | +0.6278 | Carm slightly better (no prior) |
| BIC | 5697.46 | 5696.21 | -1.26 | — |
| Entropy | 0.6940 | 0.6908 | -0.003 | Within tolerance |
| AvePP | [0.852, 0.866, 0.856] | [0.871, 0.853, 0.852] | ≤0.019 | All > 0.7 |
| ICL | 6179.55 | 6183.31 | +3.76 | — |
| Cluster sizes | [124, 171, 422] | [118, 168, 431] | ≤9 | — |
| Converged | yes | yes (142 iters) | — | — |

**Means comparison (sorted by first dimension):**

| Cluster | R mclust | Carm | Max Δ |
|---------|----------|------|-------|
...
**Analysis of differences:** R's mclust uses `priorControl()` — a conjugate prior regularization that Carm does not implement. This explains the small ΔLL = +0.6278 (Carm finds a slightly higher likelihood because it doesn't add the prior penalty). The differences in means are all < 0.02, and cluster sizes agree within 9 observations. This represents excellent agreement for a real-world dataset with overlapping clusters.

### 17.2 Engagement Entropy: N=717, K=3

| Test | R Reference | Carm | Within Tolerance | Pass |
|------|-------------|------|-----------------|------|
| Normalized entropy | 0.6940 | 0.6908 | ±0.02 | ✓ |
| Entropy range | [0, 1] | 0.6908 | — | ✓ |
| Entropy > 0.5 (not degenerate) | ✓ | ✓ | — | ✓ |
| Entropy < 0.85 (real overlap) | ✓ | ✓ | — | ✓ |
| AvePP all > 0.7 | ✓ | ✓ | — | ✓ |
| ICL internal consistency | — | BIC + 2·rawE | 1e-6 | ✓ |
| Posteriors sum to 1 | — | ∀i: Σ_k z_ik = 1 | 1e-6 | ✓ |
...
```typescript
fitLTA(data: number[][][], options: {  // N × T × M binary tensor
...
3. **Ester, M., Kriegel, H.-P., Sander, J., & Xu, X.** (1996). A density-based algorithm for discovering clusters in large spatial databases with noise. *Proceedings of the 2nd ACM SIGKDD International Conference on Knowledge Discovery and Data Mining*, 226–231.

4. **Fraley, C. & Raftery, A. E.** (2002). Model-based clustering, discriminant analysis, and density estimation. *Journal of the American Statistical Association*, 97(458), 611–631.

5. **Lance, G. N. & Williams, W. T.** (1967). A general theory of classificatory sorting strategies: I. Hierarchical systems. *The Computer Journal*, 9(4), 373–380.
...
11. **Scrucca, L., Fop, M., Murphy, T. B., & Raftery, A. E.** (2016). mclust 5: Clustering, classification and density estimation using Gaussian finite mixture models. *The R Journal*, 8(1), 289–317.
...
13. **Linzer, D. A. & Lewis, J. B.** (2011). poLCA: An R package for polytomous variable latent class analysis. *Journal of Statistical Software*, 42(10), 1–29.

14. **Biernacki, C., Celeux, G., & Govaert, G.** (2000). Assessing a mixture model for clustering with the integrated completed likelihood. *IEEE Transactions on Pattern Analysis and Machine Intelligence*, 22(7), 719–725.

---

## 20. Engineering Decisions: Problems, Solutions, and Optimizations

This section documents the engineering journey — the problems encountered during development, the decisions made, and how each was resolved. These are not textbook descriptions but hard-won insights from building a production-grade clustering library from scratch.

### 20.1 Why Eigendecomposition Instead of Direct Inversion?

**Problem:** The standard GMM E-step requires evaluating the multivariate normal log-PDF, which involves Σ⁻¹ (the precision matrix) and det(Σ). Naively computing these via matrix inversion and determinant for every observation, every component, every iteration is expensive (O(D³) per call) and numerically unstable (near-singular covariances produce wildly inaccurate inverses).

**Decision:** Store each component's covariance as its eigendecomposition Σ_k = U_k Λ_k U_k'. This was chosen over alternatives:

| Approach | Pros | Cons |
|----------|------|------|
| Direct Σ⁻¹ | Simple | O(D³) per eval, unstable near singularity |
| Cholesky L L' | Fast, stable | Must re-factor after M-step, no natural clamping |
| **Eigendecomposition** | **Natural clamping, O(D²) eval, trivial constraints** | Eigendecomposition is O(D³) per M-step |

**Why it works:** The Mahalanobis distance in the eigenbasis is just `Σ_j y_j²/λ_j` — element-wise division by eigenvalues, no matrix multiply. The log-determinant is `Σ_j log(λ_j)` — a sum, not a determinant. Constraining covariance models (diagonal, spherical, pooled) is trivial: just manipulate eigenvalues. And clamping eigenvalues to `regCovar` guarantees positive-definiteness without any special-case logic.

**Performance:** The eigendecomposition cost is amortized — it happens once per component per M-step (K times), while the E-step evaluates log-PDFs N×K times. For N=717, K=3, D=3, the E-step dominates by a factor of 717.
...
**Problem:** R's mclust uses model-based hierarchical agglomerative clustering for initialization (Fraley & Raftery, 2002). This explores a different region of the parameter space than K-Means++. On the 90-observation 3-cluster synthetic dataset, a single K-Means++ run produced ΔLL ≈ 2.0 vs mclust — acceptable, but not the exact match we wanted.

**Discovery:** Different seeds explore different basins of attraction. Running 9 seeds and keeping the best log-likelihood:

```
Seed 42:  LL = -342.013  (ΔLL = +0.004, local optimum A)
Seed 1:   LL = -342.010  (ΔLL = +0.001, closer)
Seed 7:   LL = -342.009  (ΔLL = +0.000, matches R)
...
| Model | ΔLL with 1 seed | ΔLL with 9 seeds |
|-------|----------------|------------------|
| VVV | ≈ +0.004 | ≈ +0.000 |
| EII | ≈ +0.0001 | ≈ +0.0000 |
| VVI | ≈ +0.0000 | ≈ +0.0000 |
...
```
ρ_km = (Σ_i z_ik · x_im + 1) / (N_k + 2)
```

This seemed mathematically sound — a uniform Beta prior prevents ρ from hitting exactly 0 or 1, which would cause log(0) = -∞. Cross-validation against R's poLCA revealed a ΔLL gap of 0.478 on a 2-class, 5-item dataset (N=100).

**Root cause analysis:**

```
Beta(1,1) smoothing with N_k ≈ 50:
  True ρ = 0.90 → estimated ρ = (45+1)/(50+2) = 0.885   (bias of -0.015)
  True ρ = 0.10 → estimated ρ = (5+1)/(50+2) = 0.115    (bias of +0.015)

Over 5 items × 2 classes × 100 observations, these biases compound multiplicatively
in the log-likelihood: each observation's LL shifts by Σ_m [x·log(0.885/0.90) + (1-x)·log(0.115/0.10)]
→ systematic downward bias of ~0.005 per observation → 0.478 total.
```

**Fix:** Switch to raw MLE with minimal floor instead of prior smoothing:

```
ρ_km = clip(Σ_i z_ik · x_im / N_k, [1e-10, 1-1e-10])
```

**Result:** ΔLL dropped from 0.478 to **7.6 × 10⁻⁹** — exact match to 10 decimal places. The 1e-10 floor is far enough from any realistic data value to never affect the estimate, while preventing the log(0) catastrophe.
...
**Problem:** Our initial entropy implementation used raw entropy `E = -Σ_i Σ_k z_ik · log(z_ik)`, which is unbounded (scales with N and K). Cross-validation against mclust revealed a mismatch — mclust reports entropy in [0, 1].

**Discovery:** mclust uses **normalized** entropy:

```
E_norm = 1 + Σ_i Σ_k z_ik · log(z_ik) / (N · log(K))
       = 1 - E_raw / (N · log(K))
```

The denominator N·log(K) is the maximum possible entropy (uniform posterior across K classes for all N observations). This normalizes to [0, 1] where:
...
```
ICL = BIC + 2 · E_raw          ← raw entropy
E_norm = 1 - E_raw / (N·log K)  ← normalized, reported in diagnostics
```
...
**Root cause:** poLCA requires categorical variables coded as **1/2** (not 0/1). The `probs` output contains P(value=1) and P(value=2) — where "value=2" corresponds to our original "value=1".
...
- The responsibility matrix `resp` (N × K, accessed N×K times per E-step)
- Weight and count accumulators `Nk`, `weights` (K elements, accessed N×K times)
- The distance matrix in silhouette/DBSCAN (N² elements)
...
**Problem:** HAC with Ward linkage produced heights that were systematically wrong — off by a factor that looked like a square root.

**Root cause:** R's `hclust(method = "ward.D2")` uses **squared Euclidean** distances internally for the Lance-Williams update, but reports merge heights as the **square root** of the merge distance. The `.D2` suffix stands for "D-squared" — it operates on squared distances and then takes the root.
...
**Fix:** All reference data is now extracted via `jsonlite::write_json(digits = 10)` and read directly from JSON files — no hand transcription, no copy-paste. This principle (`LEARNINGS.md`: "always extract programmatically — never type-transcribe") prevented multiple subsequent errors.
...
**Problem:** On synthetic data with well-separated clusters, Carm matches mclust exactly. On the real-world engagement dataset (717 students, overlapping clusters), there's a systematic ΔLL ≈ +0.6 (Carm slightly higher).
...
Carm uses covariance regularization (`regCovar = 1e-6` on the diagonal) but does not implement the full conjugate prior. On well-separated data the prior has negligible effect. On overlapping data it adds measurable regularization, explaining the small ΔLL.

**Decision:** Accept this difference rather than implementing `priorControl()`. The differences are small (ΔLL = 0.6 on 717 observations, means within 0.015, entropy within 0.003) and Carm's approach is more interpretable — users can control regularization explicitly via `regCovar`.
...
**Why farthest-from-own-centroid?** This point is the most poorly served by the current clustering — it's likely an outlier or a point on the boundary between clusters. Placing a new centroid there gives the empty cluster the best chance of attracting nearby points in the next assignment step.

**Alternative considered:** Random re-seeding (pick a random point). This was rejected because it doesn't exploit the structure of the problem — a random point is equally likely to be well-served or poorly-served by the current clustering.

### 20.13 Convergence Criterion Design

**Problem:** There are multiple reasonable convergence criteria for EM:
1. |ℓ(θ_t) - ℓ(θ_{t-1})| < tol (absolute log-likelihood change)
2. |ℓ(θ_t) - ℓ(θ_{t-1})| / |ℓ(θ_{t-1})| < tol (relative change)
3. max|θ_t - θ_{t-1}| < tol (parameter change)

**Decision:** Use absolute log-likelihood change (option 1) with tol = 1e-6. This matches R's mclust and GPArotation convergence criteria. Relative change (option 2) is problematic when ℓ ≈ 0. Parameter change (option 3) requires defining a norm across heterogeneous parameters (weights, means, covariances).
...
```
π: Dirichlet(1) prior — (piAcc + 1) / (N + K)
   Strength: 1 pseudo-observation per state. Mild, prevents zero initial probs.

τ: Additive smoothing 0.1 — (tauNum + 0.1) / (tauDen + 0.1·K)
   Strength: equivalent to 0.1 pseudo-transitions per cell. Very mild.
   Without: τ entries can be exactly 0, causing log(0) in forward pass.

ρ: Floor at [1e-10, 1-1e-10]
   Only prevents numerical log(0), no distributional effect.
```

These values were chosen to be small enough to not measurably affect results on reasonable datasets (N ≥ 30, T ≥ 3), while preventing the numerical catastrophes that arise on edge cases.

---

## 21. Mathematical Tricks That Made It Possible

Building a full clustering suite from scratch — no LAPACK, no BLAS, no numerical libraries — requires replacing standard library calls with mathematically equivalent formulations that are implementable in pure TypeScript. This section documents the key mathematical tricks.

### 21.1 Trick: Jacobi Eigendecomposition Instead of LAPACK

**The challenge:** GMM requires eigendecomposition of D×D covariance matrices at every M-step (6 models × K components = up to 6K decompositions per iteration). Standard implementations call LAPACK's `dsyev`, which is Fortran code unavailable in the browser.

**The trick:** Carm implements the **Jacobi eigenvalue algorithm** — iterative 2×2 rotations that zero off-diagonal elements one at a time. For a symmetric matrix A:

```
1. Find the largest off-diagonal element |A[p][q]|
2. Compute rotation angle: tan(2θ) = 2A[p][q] / (A[p][p] - A[q][q])
3. Apply Givens rotation: A ← G(p,q,θ)' · A · G(p,q,θ)
4. Accumulate eigenvectors: V ← V · G(p,q,θ)
5. Repeat until max|off-diagonal| < 1e-12
```

**Why Jacobi works here:** Covariance matrices are symmetric positive-definite (after regularization) and typically small (D ≤ 30 in most applications). Jacobi converges quadratically for symmetric matrices, requiring O(D²) sweeps of O(D²) operations each — total O(D⁴). For D=3 (engagement data), this is 81 operations per sweep, converging in 3-5 sweeps. For D=31 (factor analysis), it's ~1M operations — still sub-millisecond.
...
```
log 𝒩(x|μ,Σ) = -½[D·log(2π) + log|Σ| + (x-μ)'Σ⁻¹(x-μ)]
```

This requires Σ⁻¹ (O(D³) to compute) and |Σ| (O(D³) via LU decomposition).

**The eigendecomposition trick:** Given Σ = UΛU', where Λ = diag(λ₁,...,λ_D):

```
Σ⁻¹ = U · diag(1/λ₁,...,1/λ_D) · U'
log|Σ| = Σ_j log(λ_j)
(x-μ)'Σ⁻¹(x-μ) = Σ_j (y_j)²/λ_j    where y = U'(x-μ)
```

**Total cost:** O(D²) for the projection y = U'(x-μ), then O(D) for the sum. No matrix inversion, no determinant. The eigendecomposition is computed once per M-step and reused N times per E-step.

**Numerical bonus:** Clamping eigenvalues to `regCovar` (1e-6) simultaneously:
- Prevents log(λ) = -∞ (determinant underflow)
- Prevents 1/λ = ∞ (inverse explosion)
- Guarantees Σ is positive-definite

### 21.3 Trick: Log-Sum-Exp for Mixture Marginals

**The problem:** The marginal p(x) = Σ_k π_k · 𝒩(x|μ_k,Σ_k) involves summing probabilities that can span 300 orders of magnitude. In D=10 dimensions with well-separated clusters, p(x|k=correct) ≈ exp(-10) while p(x|k=wrong) ≈ exp(-500).

**The trick:** Work entirely in log-space using the log-sum-exp identity:

```
log p(x) = log Σ_k exp(a_k)    where a_k = log π_k + log 𝒩(x|μ_k,Σ_k)
         = max(a) + log Σ_k exp(a_k - max(a))
```

After subtracting max(a), all exponents are ≤ 0, so exp(·) ∈ (0, 1]. The sum is ≥ 1 (the max term contributes exp(0) = 1). No overflow, no underflow, full precision.

**The posterior trick:** The responsibility z_ik = exp(a_k - log p(x)) is computed by subtracting the log-marginal from the log-numerator. This is exact — no division of small numbers by small numbers.

**Where it's critical:** In LTA, the forward variable α_T(s) for a sequence of T=10 timepoints multiplies T emission probabilities and T-1 transition probabilities. Without log-space, this product would underflow to 0.0 for sequences as short as T=5.

### 21.4 Trick: Lance-Williams Recurrence for HAC

**The problem:** Naively recomputing inter-cluster distances after every merge requires O(n²·D) per merge — O(n³·D) total for n-1 merges. With n=1000 and D=10, this is 10 billion operations.

**The trick:** The Lance-Williams recurrence updates the distance between a newly merged cluster (i∪j) and any existing cluster k using only three pre-computed distances:

```
d(i∪j, k) = α_i·d(i,k) + α_j·d(j,k) + β·d(i,j) + γ·|d(i,k) - d(j,k)|
```

**Cost:** O(n) per merge (update distances to all remaining clusters), O(n²) total. This is a 10,000× speedup over naive recomputation for n=1000.
...
This requires tracking cluster sizes throughout the merge process — a detail that's easy to miss and produces subtly wrong results if forgotten.
...
```
P(selecting x_i as next center) = D²(x_i) / Σ_j D²(x_j)
where D²(x_i) = min_{c ∈ selected} ||x_i - c||²
```

Naively computing D²(x_i) requires O(N·j) distance computations when selecting the j-th center, for a total of O(N·K²).
...
```
for each i: dists[i] = min(dists[i], ||x_i - c_j||²)
```

**Cost:** O(N·K) total — one pass through all N points per center selection. The `min` operation is O(1) per point, and `dists` is pre-initialized to `Infinity`.
...
| Model | After eigendecomposing Σ̂_k = UΛU' |
|-------|--------------------------------------|
| VVV | Keep Λ, keep U |
| VVI | Keep diagonal of Σ̂_k, set U = I |
| VII | Set all eigenvalues to trace(Σ̂_k)/D, set U = I |
| EEE | Pool: Σ̂ = Σ_k π_k·Σ̂_k, eigendecompose once, share |
| EEI | Pool diagonals, share across components |
| EII | Pool trace/D, share across components |

**What this avoids:** No separate E-step implementations, no separate log-PDF functions, no conditional branching in the inner loop. The same `mvnLogPdf(x, mu, uFlat, eigenvals, d)` function handles all six models — only the eigenvalues and eigenvectors change.
...
```
v_t(s) = max_p [v_{t-1}(p) · τ_{ps}] · B_t(s)
```
...
```
log v_t(s) = log B_t(s) + max_p [log v_{t-1}(p) + log τ_{ps}]
```
...
**The problem:** Computing the cophenetic correlation requires the cophenetic distance c(i,j) — the merge height at which observations i and j first join the same cluster. Naively, this requires traversing the dendrogram for every pair — O(n²·n) = O(n³).

**The trick:** Process merges in order. For each merge of clusters A and B at height h, every pair (a,b) where a ∈ A and b ∈ B has cophenetic distance h. Using a union-find structure to track cluster membership, the cophenetic distance matrix is built in O(n²) total.

The cophenetic correlation is then the Pearson correlation between the N(N-1)/2 original distances and cophenetic distances — a standard dot-product computation.
...
```
a(i) = mean_{j ∈ same cluster, j≠i} d(i,j)
b(i) = min_{C ≠ C_i} mean_{j ∈ C} d(i,j)
```

This requires knowing which points belong to each cluster — but we already have the `labels` array.
...
|-----------|--------|-------|----------|
| GMM EM (6 covariance models) | ✓ | 263–494 | R mclust match, 3 models tested |
| GMM prediction | ✓ | 499–535 | Posterior + hard labels for new data |
| GMM model selection | ✓ | 546–564 | BIC grid search over K × model |
| GMM range fitting | ✓ | 1085–1101 | For BIC curve visualization |
| LCA (Bernoulli EM) | ✓ | 585–691 | poLCA match to 10 decimals |
| LTA (Baum-Welch + Viterbi) | ✓ | 729–950 | Structure tests, no R ref (depmixS4 non-deterministic) |
| K-Means (Lloyd + K-Means++) | ✓ | 967–1054 | R kmeans exact match |
| K-Means prediction | ✓ | 1128–1146 | Nearest centroid |
| K-Means range fitting | ✓ | 1111–1126 | For elbow plot |
| DBSCAN | ✓ | 1307–1400 | R dbscan match |
| HAC (4 linkages) | ✓ | 1489–1834 | R hclust match to 1e-10 |
| Silhouette scores | ✓ | 1199–1262 | R cluster::silhouette match |
| Distance matrix | ✓ | 1161–1181 | Float64Array, O(n²) |
| Cophenetic correlation | ✓ | 1770–1835 | R match |
| Entropy (mclust convention) | ✓ | 50–69 | Normalized, raw, case-specific |
| AvePP | ✓ | 71–83 | Per-component posterior certainty |
| PRNG (splitmix32) | ✓ | 23–35 | Deterministic, seed=42 |
| logSumExp | ✓ | 39–48 | Prevents underflow in all EM models |
...
3. **mclust priorControl():** R's mclust uses a conjugate prior regularization by default. Carm uses covariance regularization (regCovar = 1e-6) instead. This causes small differences on real-world data (ΔLL ≈ 0.6, ΔBIC ≈ 1.3 on 717-observation engagement data).

4. **Computational complexity:** The distance matrix for silhouette and DBSCAN is O(n²) in memory and time. For datasets with N > 10,000, approximate methods would be needed.
